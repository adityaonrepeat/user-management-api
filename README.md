...
**Validation is split by concern.** Whether `dob` is a well-formed date is a transport question, handled by a validator tag. Whether it is a *usable* birth date, specifically one not in the future, is a business rule handled in the service. A future date of birth produces a negative age, so rejecting it protects the one value this API exists to compute.
